...
data_table <- read_excel("rfiles/lab3/uk_soccer.xlsx")
...
pie(data_without_zeros, main = "Количество первых мест Великобритании по футболу",
...
# Первые места по 7 странам и всем видам спорта
data_table_all <- read_excel("rfiles/lab3/olympic.xlsx")

# Загрузка данных
data_table_all <- read_excel("rfiles/lab3/olympic.xlsx")

# Создаем вектор годов
years <- data_table_all$Олимпиада

# Выбираем только данные по золотым медалям (1 место) для каждой страны
...
plot(years, gold_medals[[1]], type = "o", col = "red",
     ...
     xlab = "",  # убираем подпись оси X, так как годы и так понятны
...
# Добавляем линии для остальных стран
